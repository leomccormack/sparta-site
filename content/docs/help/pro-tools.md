...
* **sparta_matrixConv** - <span style="color:orange">limited support</span>. Due to Pro Tools requiring specific fixed input/output channel configurations, the user will likely need to be creative with the bus workarounds. Binaural Ambisonic decoding applications, for example, should work fine when added as a e.g. "(5th Order Ambisonics/Stereo)" insert on an Ambisonics "Track". However, care must be taken when using other multichannel busses, since Pro Tools can reorder the channels.
